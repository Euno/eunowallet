...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("EUNO Core client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("EUNO Core"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("EUNO Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
