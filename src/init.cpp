...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: euno.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: eunod.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeeunoamount=<n> " + _("Keep N euno anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. euno is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. euno is probably already running."), strDataDir));
    ...
    if (fDaemon)
        fprintf(stdout, "Euno server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of euno") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart euno to complete") << "\n";
    ...
    nAnonymizeeunoAmount = GetArg("-anonymizeeunoamount", 0);
    if(nAnonymizeeunoAmount > 999999) nAnonymizeeunoAmount = 999999;
    if(nAnonymizeeunoAmount < 2) nAnonymizeeunoAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeeunoAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize euno Amount %d\n", nAnonymizeeunoAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1euno+1000 == (.1euno+100)*10
       10euno+10000 == (1euno+1000)*10
    */
